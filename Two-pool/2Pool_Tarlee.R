...
params <- c(0.058, 0.45, 0.5, 1, 0.1, 2.3, 0.51, 1.24, 1.2, 0.75, 0.3, 0.005, 0.2, 0.005, 0.05, 0.025, 0.023, 0.0133, 0.067, 0.01, 4.1, 40, 40, 40)
names(params) <- c("K", "c", "r_W", "r_P", "p", "h", "mu_G", "mu_W", "mu_P", "rho_G", "rho_P", "s_2nu", "s_2G", "s_2W", "s_2P", "s_2eC", "s_2eG", "s_2eW", "s_2eP", "s_2eIOM", "IOM", "Y_C1", "Y_C2", "Y_C3") 
...
observationErrorParamNames <- c("s_2eC", "s_2eG", "s_2eW", "s_2eP", "s_2eIOM")

# Names of state variables
stateNames <- c("Y_C", "Y_G", "Y_W", "Y_P", "Y_IOM")
...
for(i in 1:numFields)
{
  ...
  thisFieldData <- data[thisFieldInds, c("TOC", "Grain", "TDM", "TDM", "IOM")]
  colnames(thisFieldData) <- c("TOC", "Grain", "Wheat", "Pasture", "IOM")
...
# Auxiliary random numbers used in PMMH algorithm
...
R_initial <- matrix(0, nrow = 2, ncol = numParticles)
R_initial[1,] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = 40, sd = 10)
R_initial[2,] <- params["IOM"]


initialiseProcessModel <- function(params, R_initial)
{
  initialStateVector <- matrix(NA, nrow = 2, ncol =  numParticles)
  rownames(initialStateVector) <- c("Y_C", "Y_IOM")
  initialStateVector["Y_C",] <- R_initial[1,]        
  initialStateVector["Y_IOM",] <- R_initial[2,]
...
logLikelihoodFunction <- function(params, obs_t, y_est, observationErrorParamNames)
{
  ...
  for(i in 1:numStates)
  {
    if(!is.na(obs_t[i]))
    {
      # Log-likelihood of the SOC
      if(i == 1)
      {
        LogDens <- dlnorm(as.numeric(obs_t[1]), log(y_est[,1] + y_est[,2]), 
                          sqrt(params[observationErrorParamNames[1]]),
                          log = TRUE)
      }
      # Log-likelihood of the IOM
      else if(i == 2)
      {
        LogDens <- dlnorm(as.numeric(obs_t[2]), log(y_est[,2]), 
                          sqrt(params[observationErrorParamNames[5]]),   
                          log = TRUE)
      }
...
rProposalY <- function(currentY)
{
  ...
  StanDev = c(10, 0.9)
  for(j in 1:nrow(currentY))
  {
    for (i in 1:ncol(currentY))
    {
      theta_propY[j,i] = rtruncnorm(1, a = 0, b = Inf, mean = currentY[j,i], sd = StanDev[j])
    }
  }
...
{
  d_Y_prop = matrix(0, nrow = 2, ncol = ncol(fromY))
  StanDev = c(10, 0.9)
  for(j in 1:nrow(fromY))
  {
    for (i in 1:ncol(fromY))
    {
      d_Y_prop[j,i] = log(dtruncnorm(toY[j,i], a = 0, b = Inf, mean = fromY[j,i], sd = StanDev[j]))
    }
  }
...
logDensityPriorY <- function(inintialValueY)
{
  ...
  d_Y_prior[1] = log(dtruncnorm(inintialValueY[1], a = 0, b = Inf, mean = 40, sd = 10))
  d_Y_prior[2] = log(dtruncnorm(inintialValueY[2], mean = 4, sd = 0.5))
...
logDensityPrior <- function(theta)
{
  ...
  d_theta_prior[21] = log(dtruncnorm(theta[21], mean = 4, sd = 0.5))
...
KFtrans <- function(params,observationMatrix,numParticles, RV)
{
  ...
  NumLinearState <- stateDimension-2
...
BF <- function(params, KFtrans, numParticles, stateNames, observationMatrix, forcingMatrix, observationErrorParamNames, transitionFunction, initialiseFunction, likelihoodFunction, initialY, RV)
{
  ...
  if(any(!is.na(observationMatrix[1, ])))
  {
    ...
    weight[1, ] <- likelihoodFunction(params, observationMatrix[1, c(1,5)], t(y_est[c(1,5), , 1]), observationErrorParamNames)
  }
  ...
  for (k in 1:numParticles)
  {
    ...
    y_est[c(1,5), k, 1] = y_est_copy[c(1,5), j, 1]
    U = U + 1/numParticles
  ...
  for (t in 2:tMax) 
  {
    ...
    #Transition part
    theseStateVectors[c(1,5),] <- y_est[c(1,5), , t-1]
    ...
    {
      #Compute particle weights for this timestep
      weight[t, ] <- likelihoodFunction(params, observationMatrix[t, c(1,5)], t(y_est[c(1,5), , t]), observationErrorParamNames)
      weights[t] <- -log(numParticles) + logsumexp(weight[t, ]) + Dens3Ys[t]
    }
    else 
    {
      weight[t, ] <- log(1/numParticles) 
      y_est[c(1,5), , t] <- y_est[c(1,5), , t]
      weights[t] <- 0
    }
    ...
    for (k in 1:numParticles)
    {
      ...
      y_est[c(1,5), k, t] = y_est_copy[c(1,5), j, t]
      U = U + 1/numParticles
  ...
  # Generate indeces from multinomial Dist. to get the trajectory of the state variables
  Ind <- rep(0,tMax)
...
MH <- function(numSamples, burnIn, numParticles, KFtrans, initialParams, stateNames, observationMatrix_list, forcingMatrix_list, observationErrorParamNames, transitionFunction, initialiseFunction, logLikelihoodFunction, rProposal, logDensityProposal, logDensityPrior, rProposalY, logDensityProposalY, logDensityPriorY, R1, R_initial)
{
  ...
  tMax <- nrow(observationMatrix_list[[1]])
  storedrproposeY <- array(NA, dim = c(2,numParticles, numberOfBlocks))
  
  Y_hat_new <- array(NA, dim = c(numberOfBlocks, numSamples, 5, tMax))
  Y_hat_temporary <- array(NA, dim = c(numberOfBlocks, 5, tMax))
  ...
  currentR[,1] <- R1
  # There are two initial values that should be estimated, Y_C and Y_IOM
  numInitials <- 2
  ...
  for(i in 2:numSamples)
  {
    ...
    for(b in 1:numberOfBlocks)
    { 
      #Propose new initial "Y_C" and "Y_IOM" values and calculate the important quantities
      currentY <- y_samples[,i-1,, b] 
      names(currentY) <- c("Y_C", "Y_IOM")
    ...
    # Test the condition for acceptance of the proposed "Y_C" and "Y_IOM"
