...
params <- c(0.065, 0.45, 0.41, 1.1, 0.1, 2.1, 0.54, 2.62, 1.35, 0.5, 0.42, 0.006, 0.2, 0.01, 0.042, 0.025, 0.023, 0.133, 0.067, 0.01, 4.1, 40, 40, 40, 0.096, 0.154, 0.1, 0.743, 0.076, 0.047, 0.86, 0.9, 0.1, 0.424, 0.003, 0.84, 0.5, 0.5, 0.54, 0.11, 0.5, 0.5, 0.01, 0.5, 0.5, 0.2, 2, 0.023, 0.133)
...
transitionProcessModel <- function(params, stateVectors, forcingVariables, RV)
{
  ...
  carbonMean <- newStateVectors["Y_D", ]*exp(-1*params["KD"]) + CarbonInput     
...
R_initial[1,] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = params["Y_D1"], sd = 10)
R_initial[2,] <- rep(params["Y_IOM"],numParticles)

initialiseProcessModel <- function(params, R_initial)
{
  initialStateVector <- matrix(NA, nrow = 2, ncol =  numParticles)
  rownames(initialStateVector) <- c("Y_D", "Y_IOM")
  initialStateVector["Y_D",] <- R_initial[1,]          
...
rProposal <- function(currentTheta)
{
  ...
  theta_prop[22] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[22], sd = 1.3)
...
logDensityPrior <- function(theta)
{
  ...
  d_theta_prior[21] = log(dtruncnorm(theta[21], a = 0, b = Inf, mean = 4, sd = 0.05))
  ...
  d_theta_prior[47] = dlnorm(theta[47], 0.46, 0.6, log=TRUE) 
...
KFtrans <- function(params,observationMatrix,numParticles, RV)
{
  ...
  # yhat_zero <- c(0.5,log(2.3) + 0.5 , 1.2)
  # yhat_zero <- c(params["mu_G"],params["mu_W"], params["mu_P"])
  
  #c(params["mu_G"],log(params["h"]) + params["mu_G"], params["mu_P"])
...
{
  stateDimension <- ncol(observationMatrix) - 2
  ...
  # The Density of linear sub models
  Dens3Ys <- KFestimate$Z_density
  
  # Sampling linear state variables from Kalam filter
  ...
  if(any(!is.na(observationMatrix[1, ])))
  {
    ...
    weight[1, ] <- likelihoodFunction(params, observationMatrix[1, c(1,6)], t(y_est[c(1,6), , 1]), observationErrorParamNames)
  }
  ...
  for (k in 1:numParticles)
  {
    ...
    y_est[c(1,6), k, 1] = y_est_copy[c(1,6), j, 1]
    U = U + 1/numParticles
  ...
  for (t in 2:tMax) 
  {
    ...
    #Transition part
    theseStateVectors[c(1,6),] <- y_est[c(1,6), , t-1]
    ...
    if(any(!is.na(observationMatrix[t, ])))
    {
      ...
      #Compute particle weights for this timestep
      weight[t, ] <- likelihoodFunction(params, observationMatrix[t, c(1,6)], t(y_est[c(1,6), , t]), observationErrorParamNames)
    ...
    for (k in 1:numParticles)
    {
      ...
      y_est[c(1,6), k, t] = y_est_copy[c(1,6), j, t]
      U = U + 1/numParticles
...
MH <- function(numSamples, burnIn, numParticles, KFtrans, initialParams, stateNames, observationMatrix_list, forcingMatrix_list, observationErrorParamNames, transitionFunction, initialiseFunction, logLikelihoodFunction, rProposal, logDensityProposal, logDensityPrior, rProposalY, logDensityProposalY, logDensityPriorY, R1, R_initial)
{
  ...
  for(i in 2:numSamples)
  {
    ...
    { 
      ...
      pas["Y_D",] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = proposedTheta[b + 21], sd = 1)
      ...
      ### NOTE:
      # In the two-pool model we do not have the BIO pool
