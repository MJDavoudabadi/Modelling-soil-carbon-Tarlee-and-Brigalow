...
params <- c(0.065, 0.45, 0.41, 1.1, 0.1, 2.1, 0.54, 2.62, 1.35, 0.5, 0.42, 0.006, 0.2, 0.01, 0.042, 0.025, 0.023, 0.133, 0.067, 0.01, 4.1, 60, 60, 60, 0.096, 0.154, 0.1, 0.743, 0.076, 0.047, 0.86, 0.9, 0.1, 0.424, 0.003, 0.84, 0.5, 0.5, 0.54, 0.11, 0.5, 0.5, 0.01, 0.5, 0.5, 0.2, 2, 0.023, 0.133)
...
initialiseProcessModel <- function(params, Mean, numParticles)
{
  ...
  rownames(initialStateVector) <- c("Y_D")
...
logLikelihoodFunction <- function(params, obs_t, y_est, observationErrorParamNames)
{
  ...
  if(!is.na(obs_t))
  {
    logLike <- logLike + dlnorm(obs_t, log(y_est), 
...
rProposalY <- function(currentY)
{
  ...
  {
    theta_propY[i] = rtruncnorm(1, a = 0, b = Inf, mean = currentY[i], sd = 0.5)
  }
...
rProposal <- function(currentTheta)
{
  ...
  theta_prop[1] = rnorm(1, currentTheta[1], 0.001*2)
  ...
  theta_prop[14] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[14], sd = 0.001*5)
  ...
  theta_prop[43] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[43], sd = 0.001*50)
...
logDensityProposalY <- function(fromY, toY)
{
  ...
  d_Y_prop = log(dtruncnorm(toY, a = 0, b = Inf, mean = fromY, sd = 0.5))
...
logDensityProposal <- function(fromTheta, toTheta)
{
  ...
  d_theta_prop[1] = dnorm(toTheta[1], fromTheta[1], 0.001*2, log = TRUE)
  ...
  d_theta_prop[14] = log(dtruncnorm(toTheta[14], a = 0, b = Inf, mean = fromTheta[14], sd = 0.001*5))
  ...
  d_theta_prop[43] = log(dtruncnorm(toTheta[43], a = 0, b = Inf, mean = fromTheta[43], sd = 0.001*50))
...
{
  stateDimension <- ncol(observationMatrix) - 3
  ...
  # Sampling initial value of "Y_D"
  y_est[1, ,1] <- initialY
  
  # Sorting the samples
  for(i in 1:nrow(initialY))
  {
  ...
  # The Density of linear process models
