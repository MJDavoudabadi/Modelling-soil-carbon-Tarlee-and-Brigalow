...
# Y_Gs = The state variable of  Grain Sorghum
# Y_S = The state variable of  Sorghum
...
params <- c(0.065, 0.45, 0.41, 1.1, 0.1, 2.1, 0.54, 2.62, 1.35, 0.5, 0.42, 0.006, 0.2, 0.01, 0.042, 0.025, 0.023, 0.133, 0.067, 0.01, 12, 60, 60, 60, 0.096, 0.154, 0.1, 0.743, 0.076, 0.047, 0.86, 0.9, 0.1, 0.424, 0.003, 0.84, 0.5, 0.5, 0.54, 0.11, 0.5, 0.5, 0.01, 0.5, 0.5, 0.2, 2, 0.023, 0.133)
names(params) <- c("KD", "c", "r_W", "r_P", "p", "h", "mu_G", "mu_W", "mu_P", "rho_G", "rho_P", "s_2nu", "s_2G", "s_2W", "s_2P", "s_2TOC", "s_2eG", "s_2eW", "s_2eP", "s_2eIOM", "Y_IOM", "Y_D1", "Y_D2", "Y_D3", "s_2B", "KB", "pi_DB", "pi_BB", "KR", "KH", "PD", "s_2POC", "s_2eH", "s_2R", "s_2H", "pi_DH", "pi_RH", "pi_HH", "pi_BH", "pi_RB", "pi_HB", "r_S", "s_2S", "mu_GS", "rho_GS", "s_2GS", "h_S", "s_2eGS", "s_2eS") 
...
transitionProcessModel <- function(params, stateVectors, forcingVariables, RV)
{
  ...
  mm1 = which(stateVectors["Y_D", ]*exp(-1*params["KD"]) + params["PD"] * CarbonInput < 0)
  if (length(mm1) > 0)
  {
    CarbonInput[mm1] <- 0
  }
  
  DMean <- stateVectors["Y_D", ]*exp(-1*params["KD"]) + params["PD"] * CarbonInput
  newStateVectors["Y_D", ] <- exp(log(DMean) + sqrt(params["s_2nu"])*RV)
  
  # For the case we get negative SOC values which comes from the inputs
  # Since SOC does not get negative values the input is considered zero
  mm2 = which(stateVectors["Y_R", ]*exp(-1*params["KR"]) + (1 - params["PD"]) * CarbonInput < 0)
  if (length(mm2) > 0)
  {
    CarbonInput[mm2] <- 0
  }
  
  RMean <- stateVectors["Y_R", ]*exp(-1*params["KR"]) + (1 - params["PD"]) * CarbonInput
  ...
  if (length(mm3) > 0)
  {
    HMean[mm3] <- stateVectors["Y_H",mm3]*exp(-1*params["KH"])
  }
  ...
  if (length(mm12) > 0)
  {
    BMean[mm12] <- stateVectors["Y_B",mm12]*exp(-1*params["KB"])
  }
...
R_initial["Y_D",] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = 60, sd = 15)
R_initial["Y_IOM",] <- rep(params["Y_IOM"],numParticles)
R_initial["Y_B",] <- runif(1,0,0.05)
R_initial["Y_R",] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = 12, sd = 5)
R_initial["Y_H",] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = 30, sd = 5)
...
logLikelihoodFunction <- function(params, obs_t, y_est, observationErrorParamNames)
{
  ...
  for(i in 1:numStates)
  {
    if(!is.na(obs_t[i]))
    {
      if(i == 1)
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[i]), log(as.numeric(y_est[,1]) + as.numeric(y_est[,2]) + as.numeric(y_est[,3]) + as.numeric(y_est[,4]) + as.numeric(y_est[,5])), 
      ...
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[i]), log(as.numeric(y_est[,1]) + as.numeric(y_est[,5]) + as.numeric(y_est[,3])), 
                                    sqrt(as.numeric(params[observationErrorParamNames[8]])),   
                                    log = TRUE)
      }
      else if(i == 4)
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[i]), log(as.numeric(y_est[,4])), 
...
rProposalY <- function(currentY)
{
  ...
  {
    theta_propY[i] = rtruncnorm(1, a = 0, b = Inf, mean = currentY[i], sd = 1.3)
  }
...
rProposal <- function(currentTheta)
{
  ...
  theta_prop[12] = rtruncnorm(1, a  =0, b = Inf, mean = currentTheta[12], sd = 0.001*10)
  ...
  theta_prop[21] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[21], sd = 0.9)
  theta_prop[22] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[22], sd = 2)
  theta_prop[23] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[23], sd = 1.3)
  theta_prop[24] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[24], sd = 1.3)
  theta_prop[25] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[25], sd = 0.01*20)
  ...
  theta_prop[30] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[30], sd = 0.01*10)
  theta_prop[31] = rtruncnorm(1, a = 0, b = 1, mean = currentTheta[31], sd = 0.05*3)
  
  theta_prop[34] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[34], sd = 0.9*0.2)
  theta_prop[35] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[35], sd = 0.001*100) 
  ...
  theta_prop[45] = rtruncnorm(1, a = -1, b = 1, mean = currentTheta[45], sd = 0.05*2)
  theta_prop[46] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[46], sd = sqrt(currentTheta[46]/20))
  theta_prop[47] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[47], sd = 0.05*2)
...
logDensityProposal <- function(fromTheta, toTheta)
{
  ...
  d_theta_prop[12] = log(dtruncnorm(toTheta[12], a = 0, b = Inf, mean = fromTheta[12], sd = 0.001*10))
  ...
  d_theta_prop[21] = log(dtruncnorm(toTheta[21], a = 0, b = Inf, mean = fromTheta[21], sd = 0.9))
  d_theta_prop[22] = log(dtruncnorm(toTheta[22], a = 0, b = Inf, mean = fromTheta[22], sd = 2))
  d_theta_prop[23] = log(dtruncnorm(toTheta[23], a = 0, b = Inf, mean = fromTheta[23], sd = 1.3))
  d_theta_prop[24] = log(dtruncnorm(toTheta[24], a = 0, b = Inf, mean = fromTheta[24], sd = 1.3))
  d_theta_prop[25] = log(dtruncnorm(toTheta[25], a = 0, b = Inf, mean = fromTheta[25], sd = 0.01*20))
  ...
  d_theta_prop[30] = log(dtruncnorm(toTheta[30], a = 0, b = Inf, mean = fromTheta[30], sd = 0.01*10))
  d_theta_prop[31] = log(dtruncnorm(toTheta[31], a = 0, b = 1, mean = fromTheta[31], sd = 0.05*3))
  
  d_theta_prop[34] = log(dtruncnorm(toTheta[34], a = 0, b = Inf, mean = fromTheta[34], sd = 0.9*0.2))
  d_theta_prop[35] = log(dtruncnorm(toTheta[35], a = 0, b = Inf, mean = fromTheta[35], sd = 0.001*100))
  ...
  d_theta_prop[45] = log(dtruncnorm(toTheta[45], a = -1, b = 1, mean = fromTheta[45], sd = 0.05*2))
  d_theta_prop[46] = log(dtruncnorm(toTheta[46], a = 0, b = Inf, mean = fromTheta[46], sd = sqrt(fromTheta[46]/20)))
  d_theta_prop[47] = log(dtruncnorm(toTheta[47], a = 0, b = Inf, mean = fromTheta[47], sd = 0.05*2))
...
logDensityPrior <- function(theta)
{
  ...
  d_theta_prior[1] = log(dtruncnorm(theta[1], a = 0, b = Inf, 10, 5))   
  ...
  d_theta_prior[12] = log(dtruncnorm(theta[12], a = 0, b = Inf, mean = 0, sd = 0.5))
  d_theta_prior[13] = log(dtruncnorm(theta[13], a = 0, b = Inf, mean = 0, sd = 0.5))
  d_theta_prior[14] = log(dtruncnorm(theta[14], a = 0, b = Inf, mean = 0, sd = 0.5))
  ...
  d_theta_prior[26] = log(dtruncnorm(theta[26], a = 0, b = Inf, 0.66 , 0.3))  
  d_theta_prior[27] = dunif(theta[27], 0, 1, log=TRUE)
  d_theta_prior[28] = dunif(theta[28], 0, 1, log=TRUE)
  d_theta_prior[29] = log(dtruncnorm(theta[29], a = 0, b = Inf, 0.15 , 0.075))  
  d_theta_prior[30] = log(dtruncnorm(theta[30], a = 0, b = Inf, 0.02 , 0.01))  
  ...
  d_theta_prior[46] = log(dtruncnorm(theta[46], a = 0, b = Inf, mean = 0, sd = 0.5))
  d_theta_prior[47] = dlnorm(theta[47], 0.46, 1.6, log=TRUE) 
...
BF <- function(params, KFtrans, numParticles, stateNames, observationMatrix, forcingMatrix, observationErrorParamNames, transitionFunction, initialiseFunction, likelihoodFunction, initialY, RV)
{
  ...
  # Sampling initial values of state variables
  y_est[c(1,6,7,8,9), ,1] <- initialY
  ...
  if(any(!is.na(observationMatrix[1, ])))
  {
    ...
    weight[1, ] <- likelihoodFunction(params, observationMatrix[1, c(1,6,7,8)], t(y_est[c(1,6,7,8,9), , 1]), observationErrorParamNames)
  }
  ...
  for (k in 1:numParticles)
  {
    ...
    y_est[c(1,6,7,8,9), k, 1] = y_est_copy[c(1,6,7,8,9), j, 1]
    U = U + 1/numParticles
  ...
  for (t in 2:tMax) 
  {
    ...
    #Transition part
    theseStateVectors[c(1,6,7,8,9),] <- y_est[c(1,6,7,8,9), , t-1]
    ...
    if(any(!is.na(observationMatrix[t, ])))
    {
      ...
      #Compute particle weights for this timestep
      weight[t, ] <- likelihoodFunction(params, observationMatrix[t, c(1,6,7,8)], t(y_est[c(1,6,7,8,9), , t]), observationErrorParamNames)
    ...
    else 
    {
      ...
      y_est[c(1,6,7,8,9), , t] <- y_est[c(1,6,7,8,9), , t]
      weights[t] <- 0
    }
    ...
    for (k in 1:numParticles)
    {
      ...
      y_est[c(1,6,7,8,9), k, t] = y_est_copy[c(1,6,7,8,9), j, t]
      U = U + 1/numParticles
...
MH <- function(numSamples, burnIn, numParticles, KFtrans, initialParams, stateNames, observationMatrix_list, forcingMatrix_list, observationErrorParamNames, transitionFunction, initialiseFunction, logLikelihoodFunction, rProposal, logDensityProposal, logDensityPrior, rProposalY, logDensityProposalY, logDensityPriorY, R1, R_initial)
{
  ...
  rownames(Ini) <- c("Y_D", "Y_IOM", "Y_B", "Y_R", "Y_H")
  y_samples[,1,, ] <- Ini["Y_D",] 
