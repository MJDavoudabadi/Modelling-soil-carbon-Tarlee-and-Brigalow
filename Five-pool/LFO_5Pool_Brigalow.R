logLikelihoodFunctionLFO <- function(params, obs_t, y_est, observationErrorParamNames)
{
  ...
  for(i in 1:numStates)
  {
    if(!is.na(obs_t[i]))
    {
      if(i == 1)
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[1]), log(y_est[,1] + y_est[,6] + y_est[,7] + y_est[,8] + y_est[,9]), 
                                    sqrt(params[observationErrorParamNames[1]]),
                                    log = TRUE)
      }
      else if(2<= i & i <= 6)
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[i]), log(y_est[,i]), 
                                    sqrt(params[observationErrorParamNames[i]]),   
                                    log = TRUE)
      }
      else if(i == 8)
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[i]), log(as.numeric(y_est[,1]) + as.numeric(y_est[,8]) + as.numeric(y_est[,9])), 
                                    sqrt(as.numeric(params[observationErrorParamNames[8]])),   
                                    log = TRUE)
      }
      else if(i == 7)
      {
        logLike <- logLike + dlnorm(as.numeric(obs_t[i]), log(as.numeric(y_est[,8])), 
