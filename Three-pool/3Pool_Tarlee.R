...
params <- c(0.058, 0.45, 0.5, 1, 0.1, 2.3, 0.51, 1.24, 1.2, 0.75, 0.3, 0.005, 0.2, 0.005, 0.05, 0.025, 0.023, 0.133, 0.067, 0.01, 4.1, 40, 40, 40, 0.05, 0.05, 0.05, 0.5, 0.05)
names(params) <- c("K", "c", "r_W", "r_P", "p", "h", "mu_G", "mu_W", "mu_P", "rho_G", "rho_P", "s_2nu", "s_2G", "s_2W", "s_2P", "s_2eC", "s_2eG", "s_2eW", "s_2eP", "s_2eIOM", "IOM", "Y_C1", "Y_C2", "Y_C3", "s_2B", "KB", "pi_CB", "pi_BB", "pi_BC") 
...
stateNames <- c("Y_C", "Y_G", "Y_W", "Y_P", "Y_IOM", "Y_B")
...
transitionProcessModel <- function(params, stateVectors, forcingVariables, RV)
{
  ...
  carbonMean <- newStateVectors["Y_C", ]*exp(-1*params["K"]) + CarbonInput + stateVectors["Y_B", ]*(1-exp(-1*params["KB"])) * params["pi_BC"]
  
  mm1 = which(carbonMean<=0)
  if(length(mm1)>1){
    carbonMean[mm1] <- newStateVectors["Y_C", mm1]*exp(-1*params["K"])
  }
  
  newStateVectors["Y_C", ] <- exp(log(carbonMean) + sqrt(params["s_2nu"])*RV)
  
  BMean <- stateVectors["Y_B", ]*exp(-1*params["KB"]) + stateVectors["Y_C", ]*(1 - exp(-1*params["K"])) * params["pi_CB"]
                                                      + stateVectors["Y_B", ]*(1 - exp(-1*params["KB"])) * params["pi_BB"]
  
  mm2 = which(BMean<=0)
  if(length(mm2)>1){
    BMean[mm2] <- stateVectors["Y_B", mm2]*exp(-1*params["KB"])
  }
...
R_initial <- matrix(0, nrow = 3, ncol = numParticles)
R_initial[1,] <- rtruncnorm(numParticles, a = 0, b = Inf, mean = 40, sd = 10)
R_initial[2,] <- rep(params["IOM"],numParticles) 
R_initial[3,] <- runif(1,0,0.05)
...
initialiseProcessModel <- function(params, R_initial)
{
  ...
  rownames(initialStateVector) <- c("Y_C", "Y_IOM", "Y_B")
  initialStateVector["Y_C",] <- R_initial[1,]          
  initialStateVector["Y_IOM",] <- R_initial[2,]
  initialStateVector["Y_B",] <- R_initial[3,]*(initialStateVector["Y_C",] + initialStateVector["Y_IOM",])
...
rProposalY <- function(currentY)
{
  ...
  {
    theta_propY[j] = rtruncnorm(1, a = 0, b = Inf, mean = currentY[j], sd = 2)
  }
...
rProposal <- function(currentTheta)
{
  ...
  theta_prop[25] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[25], sd = 0.01)
  theta_prop[26] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[26], sd = 0.09)
  theta_prop[27] = rtruncnorm(1, a = 0, b = Inf, mean = currentTheta[27], sd = 0.05)
...
logDensityProposal <- function(fromTheta, toTheta)
{
  ...
  d_theta_prop[25] = log(dtruncnorm(toTheta[25], a = 0, b = Inf, mean = fromTheta[25], sd = 0.01))
  d_theta_prop[26] = log(dtruncnorm(toTheta[26], a = 0, b = Inf, mean = fromTheta[26], sd = 0.09))
  d_theta_prop[27] = log(dtruncnorm(toTheta[27], a = 0, b = Inf, mean = fromTheta[27], sd = 0.05))
...
KFtrans <- function(params,observationMatrix,numParticles, RV)
{
  ...
  y <-  matrix(0,tMax, stateDimension-2)
  y_hat <- matrix(0,tMax, stateDimension-2)
  
  Z_density <- rep(0, tMax)  
  y_est3 <- array(0,dim = c(tMax, stateDimension-2, numParticles))
...
BF <- function(params, KFtrans, numParticles, stateNames, observationMatrix, forcingMatrix, observationErrorParamNames, transitionFunction, initialiseFunction, likelihoodFunction, initialY, RV)
{
  ...
  if(any(!is.na(observationMatrix[1, ])))
  {
    ...
    weight[1, ] <- likelihoodFunction(params, observationMatrix[1, c(1,5)], t(y_est[c(1,5,6), , 1]), observationErrorParamNames)
  }
  ...
  for (k in 1:numParticles)
  {
    ...
    y_est[c(1,5,6), k, 1] = y_est_copy[c(1,5,6), j, 1]
    U = U + 1/numParticles
  ...
  for (t in 2:tMax) 
  {
    ...
    #Transition part
    theseStateVectors[c(1,5,6),] <- y_est[c(1,5,6), , t-1]
    ...
    if(any(!is.na(observationMatrix[t, ])))
    {
      ...
      #Compute particle weights for this timestep
      weight[t, ] <- likelihoodFunction(params, observationMatrix[t, c(1,5)], t(y_est[c(1,5,6), , t]), observationErrorParamNames)
      weights[t] <- -log(numParticles) + logsumexp(weight[t, ]) + Dens3Ys[t]
    }
    else 
    {
      weight[t, ] <- log(1/numParticles) 
      y_est[c(1,5,6), , t] <- y_est[c(1,5,6), , t]
      weights[t] <- 0
    }
    ...
    for (k in 1:numParticles)
    {
      ...
      y_est[c(1,5,6), k, t] = y_est_copy[c(1,5,6), j, t]
      U = U + 1/numParticles
...
MH <- function(numSamples, burnIn, numParticles, KFtrans, initialParams, stateNames, observationMatrix_list, forcingMatrix_list, observationErrorParamNames, transitionFunction, initialiseFunction, logLikelihoodFunction, rProposal, logDensityProposal, logDensityPrior, rProposalY, logDensityProposalY, logDensityPriorY, R1, R_initial)
{
  ...
  Y_hat_new <- array(NA, dim = c(numberOfBlocks, numSamples, 6, tMax))
  Y_hat_temporary <- array(NA, dim = c(numberOfBlocks, 6, tMax))
  ...
  currentR[,1] <- R1
  # Initial values that should be estimated, Y_C 
  numInitials <- 1
